...
const BinaryTreeGeneratorModal = ({ isOpen, onClose }) => {
    ...
    const handleNodeValueChange = (nodeId, value) => {
        setTreeNodes(prevNodes => prevNodes.map(node => 
    ...
    const validateTreeStructure = () => {
        ...
        if (maxDepth > 0) {
            const currentMaxDepth = Math.max(...treeNodes.map(node => node.depth));
    ...
    const handleGenerate = async () => {
        ...
        try {
            const response = await ApiRequest('generateBinaryTree', 'POST', payload);
        ...
        <div className="modal p-4 bg-white shadow-lg rounded-lg max-w-2xl mx-auto">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                ...
                <InputField
                    ...
                    type="number"
                    value={maxDepth}
                    onChange={e => setMaxDepth(Math.max(0, parseInt(e.target.value) || 0))}
                ...
                <InputField
                    ...
                    type="number"
                    value={nodeCount}
                    onChange={e => setNodeCount(Math.max(0, parseInt(e.target.value) || 0))}
            ...
            <div className="mb-6">
                <h3 className="font-semibold mb-2">Tree Structure ({treeNodes.length} nodes)</h3>
                <div className="border rounded p-4">
                    <ul className="space-y-2">
                        {treeNodes.map(node => (
                            <li key={node.id} className="flex items-center gap-2">
                                <span className="w-24">
                                    {node.parentId === null ? 'Root' : `Node ${node.id}`}
                                    <br />
                                    <span className="text-xs">(Depth {node.depth})</span>
                                </span>
                                <input
                                    type="number"
                                ...
                                />
                                {nodeErrors[node.id] && (
                                    <span className="text-red-500 text-sm">{nodeErrors[node.id]}</span>
                                )}
                                {node.children.length < 2 && (
                                    <button
                                        ...
                                        disabled={
                                            ...
                                            (nodeCount > 0 && treeNodes.length >= nodeCount)
                                        }
                                    >
                                        Add Child
                                    </button>
                ...
                {errors.nodeCount && <p className="text-red-500 mt-2">{errors.nodeCount}</p>}
                {errors.maxDepth && <p className="text-red-500 mt-2">{errors.maxDepth}</p>}
            </div>
